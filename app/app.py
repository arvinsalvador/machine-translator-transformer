...
from src.inference import InferenceError, load_inference_components, prepare_source, translate_text
...
@st.cache_resource
def cached_inference(checkpoint_mtime: float):
    """Cache only inference components; mtime invalidates after a replacement checkpoint."""
    return load_inference_components(load_config())


def translator_page(config: dict[str, Any]) -> None:
    """Final one-sentence local translator backed solely by best_model.pt."""
    st.title("English → Tagalog Translator")
    st.caption("Translation generated by the custom Transformer trained in this project.")
    checkpoint = PROJECT_ROOT / config.get("training", {}).get("checkpoint", {}).get("best_file", "models/checkpoints/best_model.pt")
    tokenizer_path = (PROJECT_ROOT / config.get("tokenizer", {}).get("model_prefix", "data/tokenizer/en_tl")).with_suffix(".model")
    ready = checkpoint.is_file() and tokenizer_path.is_file()
    if not ready:
        st.warning("The trained model is unavailable. Complete Phase 6 training first."); return
    text = st.text_area("English Text", max_chars=config.get("preprocessing", {}).get("max_characters", 500), height=120, key="translator_input")
    try:
        _, tokenizer, _ = cached_inference(checkpoint.stat().st_mtime)
        token_count = len(prepare_source(text, tokenizer, config["model"]["max_sequence_length"], config["preprocessing"]["max_characters"])) if text.strip() else 0
        st.caption(f"Characters: {len(text)} · Tokens: {token_count} / {config['model']['max_sequence_length']}")
    except InferenceError as error: st.error(str(error)); return
    if st.button("Translate", type="primary", disabled=not text.strip()):
        try:
            result = translate_text(text, config, cached_inference(checkpoint.stat().st_mtime))
            st.subheader("Tagalog Translation"); st.success(result["translation"] or "(No text generated)")
            cols = st.columns(4); cols[0].metric("Source tokens", result["source_token_count"]); cols[1].metric("Output tokens", result["generated_token_count"]); cols[2].metric("Device", result["device"]); cols[3].metric("Time", f"{result['duration_ms']:.0f} ms")
            history = st.session_state.setdefault("translation_history", []); history.insert(0, {"English": result["source_text"], "Tagalog": result["translation"], "Time": f"{result['duration_ms']:.0f} ms"}); del history[10:]
        except InferenceError as error: st.error(str(error))
    if st.session_state.get("translation_history"):
        st.subheader("Session Translation History")
        if st.button("Clear History"): st.session_state["translation_history"] = []
        else: st.dataframe(st.session_state["translation_history"], use_container_width=True, hide_index=True)
    st.info("This is a small academic Transformer trained on limited caption-style data. Quality can vary; dataset Tagalog references are machine-generated, not manually verified human translations.")
...
def main() -> None:
    ...
    config = load_config()
    page = st.sidebar.radio("Navigation", ("Dashboard", "Dataset", "Data Preparation", "Tokenizer", "Model", "Training", "Evaluation", "Translator", "About"))
    if page == "Dashboard":
        dashboard(config)
    elif page == "Dataset":
        dataset_page(config)
